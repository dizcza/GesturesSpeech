...
isMultipleName = lambda item: type(item).__name__ == "MultipleActionName"


class MultipleActionName(object):
    ...
    def __eq__(self, other):
        """
        :param other: instance of class "str" or "MultipleActionName"
        :return: valid names intersection
        """
        intersection = False
        for self_name in self.valid_names:
            if isMultipleName(other):
...
def resave_face_structure_json():
    ...
    modif = {
        "52-3-1": ("mouth", ["default"]),
        "49-1-2": ("cheeks", ["up"]),
        "37-1-1": ("cheeks", ["default"]),
        "56-2-2": ("cheeks", ["up"]),
        "46-4-1": ("eyebrows", ["down"])
    }
    for fname in modif:
        face_area, ok_act = modif[fname]
