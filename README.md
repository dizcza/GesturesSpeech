...
<p>The main idea in gesture recognition is to maximize between class variance <span lang="latex">D_b</span> and minimize within class variance <span lang="latex">D_w</span> by choosing appropriate hidden parameters (training step). For this purpose Weighted DTW algorithm has been <a href="http://datascience.sehir.edu.tr/pub/VISAPP2013.pdf">proposed</a>.</p>
...
<p>Using the total displacement values of joints, the joint <span lang="latex">j</span>'s weight value of class <i>g</i> is calculated via
<div lang="latex" align="center">
\omega_j^g = \frac{1-e^{-\beta D_j^g}}{\sum_{k}(1-e^{-\beta D_k^g})}
</div>
Best beta yields the biggest discriminant ratio <span lang="latex">R = \rfrac{D_b}{/D_w}</span>. In our case, although within variance <span lang="latex">D_w</span> goes up with the <span lang="latex">\beta"</span> decreasing, the <span lang="latex">R_{max}</span> is obtained when  <span lang="latex">\beta"</span> vanishes. That means,</p>
<div lang="latex" align="center">\omega_j^g \xrightarrow{\beta \to 0} \frac{D_j^g}{\sum_{k}D_k^g}</div>
